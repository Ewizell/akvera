      ...
      <div className="flex items-start gap-1.5 py-1">
        {hasChildren && (
          <button
            ...
            className="mt-0.5 shrink-0 flex h-4 w-4 items-center justify-center rounded border border-gray-300 text-[10px] leading-none text-gray-400 hover:border-[#179146] hover:text-[#179146] transition-colors"
          >
            {isOpen ? "−" : "+"}
          </button>
        )}
        ...
        <Link
          href={path}
          onClick={onNavigate}
          className={`min-w-0 flex-1 break-words text-sm hover:text-[#179146] transition-colors ${
            depth === 0 ? "text-[#28313d]" : "text-[#64748b]"
          }`}
        >
          {category.name}
        </Link>
...
export default function CatalogMenu({ categories }: { categories: CategoryNode[] }) {
  ...
  return (
    <div className="shrink-0" ref={ref}>
      ...
      <div
        className="hidden md:flex absolute left-1/2 top-[calc(100%+8px)] w-[940px] lg:w-[1100px] max-w-[calc(100vw-32px)] h-[80vh] bg-white rounded-xl shadow-lg border border-gray-100 z-50 origin-top overflow-hidden"
        style={{
          opacity: open ? 1 : 0,
          transform: open
      ...
      >
        <div className="w-[240px] lg:w-[280px] shrink-0 border-r border-gray-100 overflow-y-auto py-2">
          {categories.map((cat) => {
              ...
              >
                ...
                >
                  ...
                  {typeof cat.productCount === "number" && (
                    <span className="shrink-0 text-xs text-gray-400">{cat.productCount}</span>
                  )}
                </Link>
          ...
          {activeRoot && activeRoot.children.length > 0 ? (
            <>
              <Link
                ...
                onClick={close}
                className="group mb-4 flex items-center gap-2 text-lg font-bold text-[#28313d] hover:text-[#179146] transition-colors"
              >
                {activeRoot.name}
                <svg
                  width="14"
                  ...
                  stroke="currentColor"
                  strokeWidth="2"
                  className="shrink-0 text-gray-400 transition-colors group-hover:text-[#179146]"
                >
                  <path d="m7 4 6 6-6 6" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </Link>
      ...
      {/* ───────── Мобильные и планшетные экраны: полноэкранная панель-аккордеон ───────── */}
      <div
        className={`md:hidden fixed inset-x-0 bottom-0 top-[60px] z-50 bg-white overflow-y-auto transition-transform duration-300 ${
          open ? "translate-x-0" : "translate-x-full pointer-events-none"
        }`}
        style={{ transitionTimingFunction: transitionCurve }}
      >
